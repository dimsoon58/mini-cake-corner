...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-10 w-full items-center justify-between gap-2 overflow-hidden rounded-md border border-input bg-background px-3 py-2 text-center text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:block [&>span]:min-w-0 [&>span]:flex-1 [&>span]:text-center [&>span]:truncate [&>span]:whitespace-nowrap [&>span_img]:hidden [&>span_div]:block [&>span_div]:w-auto",
      className,
    )}
    {...props}
  >
...
/* `itemText` : libelle court repris dans le champ une fois l'option choisie.
   Sans lui, tout le contenu de l'option (descriptions, allergenes...) serait
   recopie dans le champ ferme et deborderait. */
const SelectItem = React.forwardRef<
  React.ElementRef<typeof SelectPrimitive.Item>,
  React.ComponentPropsWithoutRef<typeof SelectPrimitive.Item> & { itemText?: React.ReactNode }
