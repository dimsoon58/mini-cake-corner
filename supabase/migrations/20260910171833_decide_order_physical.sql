...
-- This RPC does the WHOLE decision in ONE transaction, under row locks:
--   * lock the orders row + the action-token row (FOR UPDATE)
--   * verify the token is unused
--   * verify physical_validation = 'pending'
--   * verify workshop_confirmed_at IS NOT NULL for a mixed order
--   * run invariant checks on the refund amount
--   * write exactly ONE decision (order_validation / physical_validation /
--     refund_status / refund_due_amount) — which fires
--     handle_order_reward_status_change() in the same transaction
--   * finalise (approve) or release (reject) the welcome-discount reservation
--   * consume the token
-- ...or raise, leaving everything untouched.
--
-- A retry of the WINNING request (same token, now used, order already decided)
-- returns { already_decided: true, ... } instead of erroring, so manage-order
-- can still send the e-mail / invoice / Make webhook idempotently.
--
-- manage-order does the non-transactional side-effects (invoice PDF, e-mails,
-- Make status webhook) AFTER this returns.
